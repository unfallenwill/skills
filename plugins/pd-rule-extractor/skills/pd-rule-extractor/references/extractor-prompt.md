...
- 每格必须落盘文件且非空：要么至少一条候选规则行，要么一行确认无哨兵行。空文件视为未执行（拆通道模式下以该格全部通道合并后的状态判定，单通道任务本身不受此条约束）。
...
返回简短摘要：产出候选规则条数（或确认无）、扫描中发现的疑似缺口（如引用外部手册无法展开的条目）、未解析数据点清单（如有）。
...
## 独立任务：变量字典提取（阶段 1 只派发一次）

**本任务不随矩阵格派发。** 主执行者在阶段 1 仅派发一次本任务。从数据库设计说明/CRF 文档的 chunks 中提取全部变量定义，写入 `.pd-extraction/variables.jsonl`（一行一个 JSON 对象，UTF-8）：

```json
{"dataset": "<数据集名（与判定逻辑引用一致，如 DM、VISIT）>", "variable": "<变量名>", "label": "<变量标签/中文名>", "form": "<所属 CRF 表单>"}
```

要求：

- 覆盖该文档中全部数据集/表单的全部变量，一条不漏；同一变量在多表单出现时按表单逐条记录。
- 数据集名与变量名照录原文大小写；文档只有中文名时，`variable` 填中文名、label 同填（下游匹配大小写与分隔符不敏感）。
- 数据库设计文档为 xlsx 时：本任务产物用于语义理解（label、表单归属），代码级存在性核实由 verify_variables.py 直接读 sheet 索引完成，两者不互相替代。
- 数据库设计文档缺失时：不创建该文件，在返回摘要中注明。
- 返回摘要：数据集数、变量总数、无法确定归属表单的变量数。
